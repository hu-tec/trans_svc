...
//AES256 복호화 필요 파라미터
$DECRYPT_PARAMS = array("mchtCustId","trdAmt", "pointTrdAmt", "cardTrdAmt", "vtlAcntNo", "cphoneNo" );
...
function sendResult()
{
    if(top.opener){
        ...
        parent.rstparamSet(_PAY_RESULT);
        parent.goResult();
    }
}

if(_PAY_RESULT.outStatCd=="0021") {
    alert("결제가 완료되었습니다.");
    sendResult();
} else {
    alert("결제에 실패하였습니다. 오류코드["+_PAY_RESULT.outRsltCd+"] 오류메세지["+_PAY_RESULT.outRsltMsg+"]");
    sendResult();
}
